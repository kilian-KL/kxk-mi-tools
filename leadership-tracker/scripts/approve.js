 ...
 * nach public/tracker-data.json. Publikation NUR über diesen Schritt —
...
if (argv[0] === "--remove") {
  ...
  pub.entries = pub.entries.filter(e => !e.person.toLowerCase().includes(name));
  save(true);
  console.log(`Entfernt: ${before - pub.entries.length} Eintrag/Einträge zu "${argv[1]}".`);
  process.exit(0);
}
...
(function next() {
  ...
  console.log(`\n${e.date} · ${e.person} — ${e.role}, ${e.company}${e.city ? ", " + e.city : ""}`);
  console.log(`  "${e.summary}"  [${e.source}] ${e.url}`);
